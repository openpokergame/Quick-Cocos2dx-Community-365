...
namespace cocostudio
{
    ...
    void ImageViewReader::setPropsWithFlatBuffers(cocos2d::Node *node, const flatbuffers::Table *imageViewOptions)
    {
        ...
        // }
        Scale9Sprite *sprite = static_cast<Scale9Sprite*>(node);
        auto options = (SpriteOptions*)imageViewOptions;
        auto fileNameData = options->fileNameData();
        int resourceType = fileNameData->resourceType();

        auto options_1 = (ImageViewOptions*)imageViewOptions;
        bool scale9Enabled = options_1->scale9Enabled();
        switch (resourceType)
        {
            case 0:
            {
                ...
                if (path != "")
                {
                    ...
                    if(scale9Enabled){
                        auto f_capInset = options_1->capInsets();
                        Rect capInsets(f_capInset->x(), f_capInset->y(), f_capInset->width(), f_capInset->height());
                        sprite->initWithFile(capInsets,path);
                    }else{
            ...
            case 1:
            {
                ...
                if (path != "")
                {
                    ...
                    if(scale9Enabled){
                        auto f_capInset = options_1->capInsets();
                        Rect capInsets(f_capInset->x(), f_capInset->y(), f_capInset->width(), f_capInset->height());
                        sprite->initWithSpriteFrameName(path,capInsets);
                    }else{
        ...
        if(scale9Enabled){
            auto f_scale9Size = options_1->scale9Size();
            Size scale9Size(f_scale9Size->width(), f_scale9Size->height());
            sprite->setContentSize(scale9Size);
        }
    ...
    Node* ImageViewReader::createNodeWithFlatBuffers(const flatbuffers::Table *imageViewOptions)
    {
        ...
        setPropsWithFlatBuffers(sprite, (Table*)imageViewOptions);
